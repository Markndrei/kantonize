    ...
    <header className="w-full px-4 md:px-[4rem]">
        ...
        <div className="w-full p-2 absolute top-0 left-0 bg-white shadow-md grid gap-2">
					<div className="w-full flex justify-between items-center px-2">
						<Link href="/" className="text-2xl font-bold text-gray-800">KANTONIZE</Link>
						<div className="text-2xl" onClick={() => setOpenNavBar(prev => !prev)}>X</div>
					</div>
					<div className="p-2 flex justify-center border hover:bg-gray-400" onClick={() => setOpenNavBar(prev => !prev)}>
					<Link href="/" className="text-gray-600 uppercase">Home</Link>
					</div>
					<div className="p-2 flex justify-center border hover:bg-gray-400" onClick={() => setOpenNavBar(prev => !prev)}>
					<Link href="/kantonize" className="text-gray-600 uppercase">Kantonize</Link>
					</div>
					<div className="p-2 flex justify-center border hover:bg-gray-400" onClick={() => setOpenNavBar(prev => !prev)}>
					<Link href="/about" className="text-gray-600 uppercase">About Us</Link>
					</div>
					<div className="p-2 flex justify-center border hover:bg-gray-400" onClick={() => setOpenNavBar(prev => !prev)}>
					<Link href="/contact" className="text-gray-600 uppercase">Contact Us</Link>
					</div>
				</div>
      ) : (
        false
